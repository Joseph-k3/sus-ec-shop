...
/**
 * Square APIで決済状況を直接確認するスクリプト (SDK v43対応)
 * 
 * 使い方:
 * node scripts/check-square.cjs
 */
...
const client = new square.SquareClient({
  ...
  environment: IS_SANDBOX ? square.SquareEnvironment.Sandbox : square.SquareEnvironment.Production,
})
...
async function checkSquareOrders() {
  ...
  try {
    const response = await square.orders.searchOrders({
    ...
    console.log(`📦 見つかった注文: ${response.orders.length}件\n`)
...
async function checkSquarePayments() {
  ...
  try {
    const response = await square.payments.listPayments({
    ...
    console.log(`💳 見つかった決済: ${response.payments.length}件\n`)
    console.log('━'.repeat(80))
...
async function checkPaymentLinks() {
  ...
  try {
    const response = await square.checkout.listPaymentLinks({
      limit: 10
    }, {
      accessToken: SQUARE_ACCESS_TOKEN,
      environment: IS_SANDBOX ? square.SquareEnvironment.Sandbox : square.SquareEnvironment.Production,
    ...
    if (!response.paymentLinks || response.paymentLinks.length === 0) {
      console.log('🔗 決済リンクが見つかりませんでした\n')
      return
    }

    console.log(`🔗 見つかった決済リンク: ${response.paymentLinks.length}件\n`)
    console.log('━'.repeat(80))

    response.paymentLinks.forEach((link, index) => {
